...
resource "aws_acm_certificate" "server" {
  ...
  certificate_chain = tls_self_signed_cert.ca.cert_pem        # Chain includes the CA cert for completeness
...
resource "aws_ec2_client_vpn_endpoint" "endpoint" {
  ...
  transport_protocol            = "tcp"                       # TCP/443 is commonly chosen for firewall traversal; UDP often offers better throughput
  vpn_port                      = 443
